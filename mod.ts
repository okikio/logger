import type { LogRecord, Logger as LogTapeLogger, Sink } from "@logtape/logtape";
...
export type LogLevel = "error" | "debug" | "fatal" | "info" | "warn";
...
export class Logger<L extends string = "general"> {
  ...
  #log(level: LogLevel, _message: unknown, ...args: unknown[]): void {
    ...
    } else if (typeof _message === "object" && !Array.isArray(_message)) {
      message = this.#message as string || "Log message"; // Default message if no template is provided
      Object.assign(context, _message as Record<string, unknown>);

      const possibleArg = typeof args[0] === "function" ? args[0]?.(context) : args[0];
      if (possibleArg) Object.assign(context, (possibleArg ?? {}) as Record<string, unknown>);
      logger[level](message, context);
    } else {
      logger[level](_message as TemplateStringsArray, ...args);
    }
  }
